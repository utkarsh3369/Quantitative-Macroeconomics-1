...
% Note that the AR(1) process is not i.i.d. (it is NOT independently distributed)
% =======================================================================
...
sig_z = 0.2; mu_z = 10;       % parameters for normal distribution
...
nu1 = 8; mu_st1 = 0;          % parameters for student t with finite variance
...
subplot(2,3,4); 
    ...
    title('Gamma errors');
subplot(2,3,5); 
    plot(Y(:,5),'linewidth',2);  
    line(0:T,repmat(mu_y,1,T+1),'linestyle','--','color','black'); 
    title('Student''s t finite variance errors');
subplot(2,3,6); 
    plot(Y(:,6),'linewidth',2);
    line(0:T,repmat(mu_y,1,T+1),'linestyle','--','color','black'); 
