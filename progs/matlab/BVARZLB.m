...
% monetary policy is at the zero lower bound (i.e. unlikely to respond to
% changes in other variables).
% =========================================================================
% BVARZLB(prior_adjust_for_ZLB)
% -------------------------------------------------------------------------
% INPUTS
%	- prior_adjust_for_ZLB : boolean, 1: adjust prior variance to reflect the view that  monetary policy is at the zero lower bound
% -------------------------------------------------------------------------
% OUTPUTS
...
% -------------------------------------------------------------------------
% Willi Mutschler, January 19, 2023
% willi@mutschler.eu
% =========================================================================
...
% hyperparameters for Minnesota prior for BVAR model
...
nsave = 10000;          % dinal number of draws to keep
...
% Now define matrix Z which has all the right-hand-side variables (constant, lags of the dependent variable and exogenous regressors/dummies) and also get rid of NA observations
if const == 0
...
if prior_adjust_for_ZLB
    % manually adjust for zero-lower bound on nominal interest rates. For this,
    % we need to focus on coefficients A1_12, A1_13, A1_14, A2_12, A2_13, A2_14
    % as these are the reaction coefficients of the other variables on the Federal Funds Rate
    % our prior mean already sets them equal to 0, but we additionally want
    % to use a very small prior variance to reflect the view that we are
    % quite sure that these parameters are very close to zero, i.e. monetary
    % policy is constrained by the zero-lower bound on nominal interest rates.
    
    % find position of A1_12, A1_13, A1_14, A2_12, A2_13, A2_14
    I_K = eye(K); tmp = zeros(K,K); tmp(1,2:K) = 1;
...
disp(reshape(alpha_prior,K,1+2*K));   % assume a Random Walk prior mean
...
disp(reshape(diag(V_prior),K,1+2*K)); % uncertainty of the Random Walk assumption
...
% initialize first draw of SIGMAU with OLS value
...
for j = 1:ntot
    if mod(j,1000) == 0
        waitbar(j/ntot); % ipdate waitbar every 1000th step
    end
    % posterior of alpha_j|SIGMAU,Y ~ N(alpha_post,V_post)
    invSIGMAU_j = inv(SIGMAU_j);
    V_post = (inv_V_prior + kron(ZZt,invSIGMAU_j))\eye(size(inv_V_prior,1));  % conditional posterior covariance matrix for alpha
    alpha_post = V_post*(inv_V_prior*alpha_prior + kron(Z,invSIGMAU_j)*Y(:)); % conditional posterior mean for alpha
    ...
    % Posterior of SIGMAU_j|alpha_j,Y ~ invWishard(inv(S_post),v_post)
    resid_irep = Y - A_j*Z;                      % residuals conditional on draw of alpha_j
    v_post = T + v_prior;                        % conditional posterior degrees of freedom for SIGMAU
    S_post = S_prior + resid_irep*resid_irep';   % conditional posterior scale matrix for SIGMAU
    SIGMAU_j = inv(wishrnd(inv(S_post),v_post)); % draw of SIGMAU_j

    % Store results if burn-in phase is passed
    if j > nburn               
        A_draws(:,:,j-nburn) = A_j;           % store A_j
        SIGMAU_draws(:,:,j-nburn) = SIGMAU_j; % store SIGMAU_j
    end       
end
